...
public class EmployeeServiceImpl implements EmployeeService {
  ...
  public EmployeeServiceImpl(EmployeeDao ed, BCryptPasswordEncoder passwordEncoder) {
    ...
    System.out.print("ed " + ed.toString() + ", ps " +  passwordEncoder.toString());
  }
    ...
    for (i = 0; i < careerList.size(); i++) {
      ...
      if (projectList != null) {
        for (int j = 0; j < projectList.size(); j++) {
          ...
          if (((Project)projectList.get(j)).getProject_no() > 0 && ((Project)projectList.get(j)).getEnd_date().equals(""))
