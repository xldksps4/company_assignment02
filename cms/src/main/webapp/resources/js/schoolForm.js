...
$(document).off("change", ".qualification").on("change", ".qualification",
		function() {
			qualification();
		});
...
function qualification() {
	if ($(".qualification").is(":checked")) {
		$("#highSchool").find("input").not(".qualification").not(
...
// 학교명 입력 시 리스트에 추가
$(document).off("blur", "[name=school_name]").on("blur", "[name=school_name]",
		function() {
			schoolList();
		});
...
function schoolList() {
	...
	$("#schoolArea [name=school_name]").each(
			function() {
...
// 해당 학력사항 삭제
$(document).off("click", ".deleteSchool").on(
		"click",
		".deleteSchool",
		function() {
			var school_no = $(this).parents("form").find("[name=school_no]")
					.val();
...
$(document).on(
		...
		function() {
			...
			// 입학일 - 졸업일 기간 범위 체크
			var start = $(this).parents(".fields")
					.find("[name=admission_date]").val();
			start = Number(start.replace(".", ""));
			var end = $(this).parents(".fields").find("[name=graduate_date]")
					.val();
			end = Number(end.replace(".", ""));

			if (start > end) {
				$(this).parents(".fields").find("[name=graduate_date]").parent(
						"div").addClass("error");
			} else {
				$(this).parents(".fields").find("[name=graduate_date]").parent(
	...
	$("#schoolArea .schoolForm")
			.each(
					function(index) {
						...
						var admission_date = $(this).find(
								"[name=admission_date]");
						var graduate_date = $(this)
								.find("[name=graduate_date]");
						var major = $(this).find("[name=major]");

						if (index == 0
								&& $(this).find(".qualification")
										.is(":checked")) {
							checkResult = true;
						} else {
							if (school_name.val() == "") {
								...
								return false;
							} else if (graduate_date.val().length <= 0
									|| graduate_date.parent("div").hasClass(
											"error")) {
